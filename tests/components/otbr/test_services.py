...
def mock_pending_endpoint(
    aioclient_mock: AiohttpClientMocker, in_flight: str | None = None
) -> None:
...
async def test_in_flight_pending_dataset_is_outstamped(
    hass: HomeAssistant,
    otbr_config_entry_multipan: str,
    aioclient_mock: AiohttpClientMocker,
) -> None:
    """A migration already in flight does not swallow a newer one.

    The new pending dataset must carry timestamps above the in-flight one,
    or the mesh ignores it as not-newer while the action reports success.
    """
    in_flight = dict(tlv_parser.parse_tlv(TARGET))
    in_flight[MeshcopTLVType.PENDINGTIMESTAMP] = Timestamp.from_values(
        MeshcopTLVType.PENDINGTIMESTAMP, seconds=5000
    )
    mock_pending_endpoint(aioclient_mock, in_flight=tlv_parser.encode_tlv(in_flight))

    await call_migrate(hass, dataset=TARGET)

    puts = pending_calls(aioclient_mock)
    assert tlv_parser.parse_tlv(puts[0][2]) == expected_pending(TARGET, 5001, 300000)
...
async def test_in_flight_migration_away_is_superseded(
    hass: HomeAssistant,
    otbr_config_entry_multipan: str,
    aioclient_mock: AiohttpClientMocker,
) -> None:
    """Targeting the current network is not a no-op while a move away is queued."""
    ...
    puts = pending_calls(aioclient_mock)
    assert len(puts) == 1
    # Stamped above the queued migration (ts 1003), keeping this network.
...
async def test_ticks_count_in_the_comparison(
    ...
    in_flight = dict(tlv_parser.parse_tlv(TARGET))
    in_flight[MeshcopTLVType.PENDINGTIMESTAMP] = Timestamp.from_values(
        MeshcopTLVType.PENDINGTIMESTAMP, seconds=1003, ticks=42
    )
    mock_pending_endpoint(aioclient_mock, in_flight=tlv_parser.encode_tlv(in_flight))
...
async def test_migration_reports_a_discarded_store_write(
    ...
    async def store_newer_dataset(dataset: bytes) -> None:
