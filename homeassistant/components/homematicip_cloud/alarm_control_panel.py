...
class HomematicipAlarmControlPanelEntity(AlarmControlPanelEntity):
    ...
    def _publish_problems(self, result: RestResult) -> list[str]:
        """Publish the devices that refused the request and return them."""
    ...
    async def async_arm_anyway(self, mode: str) -> None:
        ...
        approved = set(self._hap.arming_problems)
        ...
        # Ask again and refuse when anything else has opened since.
        result = await self._home.set_security_zones_activation_async(internal, True)
        problems = self._publish_problems(result)
        if not problems:
            # armed after all, or refused without naming a device
            self._raise_for_result(result)
            return

        if not approved.issuperset(problems):
            raise HomeAssistantError(
                translation_domain=DOMAIN,
