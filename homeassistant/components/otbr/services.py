...
async def _async_repoint_preferred_dataset(
    ...
    for entry in store.datasets.values():
        if entry.extended_pan_id.lower() == source_extended_pan_id.lower():
            ...
            target_id = entry.id
    if source_id and target_id and store.preferred_dataset == source_id:
...
async def _pinned_channel_of_another_router(
    ...
    on, so the common setup pays for no extra calls, and one that cannot be
    reached is left out rather than blocking the migration.
    """
    ...
    for other in hass.config_entries.async_loaded_entries(DOMAIN):
        ...
        try:
            ...
            _LOGGER.debug(
                "Could not read the network of %s, not checking its channel",
                other.title,
            )
            continue
...
async def _async_migrate_network(call: ServiceCall) -> dict[str, Any]:
    ...
    async with async_get_dataset_lock(call.hass):
        ...
        in_flight = None
        if pending_tlvs := await data.get_pending_dataset_tlvs():
            try:
        ...
        # pending dataset in flight the mesh is about to leave this
        ...
        # being left AND any migration already in flight: a not-newer
        # pending dataset is silently ignored by the mesh while this
        # action would still report success.
        newest = max(
